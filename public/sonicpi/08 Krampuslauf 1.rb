...
define :play_notes do |seed, source, tempo, pan, octave, loops, enable_midi, rotate|
  with_random_seed seed do
    with_random_source source do
      notes = (scale :A2, :augmented2).reflect.butlast
      beats = (spread notes.length - 3, notes.length).rotate(rotate)
      with_synth :fm do
...
uncomment do
  in_thread do
    play_notes 20231219, :pink, 180, 0.5, -1, 20, false, 0
  end
end

uncomment do
  in_thread do
    play_notes 19122023, :pink, 180, 0.0, 0, 20, false, 1
  end
end

uncomment do
  in_thread do
    play_notes 12192023, :pink, 180, -0.5, 1, 20, true, 2
  end
end
